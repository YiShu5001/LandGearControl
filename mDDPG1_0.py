...
for episode in range(num_episodes):
    ...
    for t in range(8000):
        ...
        A0_L.append(action)
        A_L.append(action1)
        next_state, reward, done, _ = env.step(action1, 4, 5e-2, 100e-3)
        #存入全部数据还是仅调平数据
    ...
    # if episode_reward >63000:
    #     env.ADRC_control.show()
    #     env.save(episode, A_L, Re_L, Rt_L,name=str(T_skip))
    #     name1 = str(T_skip)+'_'+str(episode)+'actor1.pth'
    #     name2 = str(T_skip)+'_'+str(episode)+'critic1.pth'
    #     torch.save(Actor,name1)
    #     torch.save(Critic,name2)
